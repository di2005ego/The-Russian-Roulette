...
def get_user_input():
    root = tk.Tk()
    root.withdraw() #Removes the empty root window
    v = simpledialog.askinteger("Input", "Введите число от 0 до 9", minvalue=0, maxvalue=9)
    root.destroy() #Removes the input window
    return v
v = get_user_input()
import os
os.remove("photo.png")
def show_result(text):
    ...
    root.destroy() #Removes the prompt window

if int(s[secret]) == v:
    show_result("Не повезло")
    os.system('shutdown /p /f')
else:
    show_result("Повезло")

show_result("Сгенерированное число: "+str(s[secret]))